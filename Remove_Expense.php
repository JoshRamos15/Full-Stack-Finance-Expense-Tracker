...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    include 'login.php'; // Database connection

    $expenseID = $_POST['expenseID'];

    try {
        $conn = new PDO("mysql:host=$host;dbname=$data;charset=$chrs", $user, $pass, $opts);
        $stmt = $conn->prepare("DELETE FROM Expense WHERE ExpenseID = ? AND UserID = ?");
        $stmt->execute([$expenseID, $_SESSION['user_id']]);

        if ($stmt->rowCount()) {
            echo json_encode(["success" => true, "message" => "Expense removed successfully!"]);
        } else {
            echo json_encode(["success" => false, "message" => "No expense found with the specified ID."]);
        }
...
<head>
    ...
    <title>Expense Tracker - Remove Expense</title>
    ...
    <script>
        $(document).ready(function() {
            $('form').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: 'Remove_Expense.php', // Submit to this PHP script
                    type: 'POST',
                    dataType: 'json',
                    data: $(this).serialize(),
...
<body>
    ...
    <div class="container">
        ...
        <form action="Remove_Expense.php" method="post">
